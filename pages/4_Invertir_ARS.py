...
if st.button("Generar recomendación", type="primary", use_container_width=True):
    ...
    # Separar picks antes de las tabs
    ...
    en_cartera   = [p for p in (cedear_picks or []) if p["ticker"].upper() in alloc_text]
    alternativas = [p for p in (cedear_picks or []) if p["ticker"].upper() not in alloc_text]

    alloc_merval_text = " ".join(
        (pos.get("instrument_id", "") + " " + pos.get("name", "")).upper()
        for pos in allocation if pos.get("type") == "accion_merval"
    )
    en_cartera_m   = [p for p in (merval_picks or []) if p["ticker"].upper() in alloc_merval_text]
    alternativas_m = [p for p in (merval_picks or []) if p["ticker"].upper() not in alloc_merval_text]
    ...
    st.divider()
    tab_resumen, tab_detalle = st.tabs(["📊 Resumen ejecutivo", "🔍 Análisis detallado"])
    ...
    with tab_detalle:
        ...
        if risk_rows:
            ...
            st.caption("⚠️ No incluye comisiones, spread ni diferencia entre precio teórico y ejecutado.")
        ...
        if has_cedears and cedear_picks:
            ...
            if alternativas:
                ...
                st.caption("Buen score pero descartados en este contexto. Consideralos como alternativas.")
        ...
        if has_merval and merval_picks:
            ...
            if alternativas_m:
                ...
                st.caption("Score ≥ 6 pero el advisor no las seleccionó. Seguirlas como alternativas.")
